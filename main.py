...
try:  # Check if the requrements have been installed
    from discord_webhook import DiscordWebhook, DiscordEmbed  # Try to import discord_webhook
    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Generator and Checker - Made by LTZ999")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by LTZ999\a',
...
╚══════╝╚═╝   ╚══════╝ ╚════╝  ╚════╝  ╚════╝
        ...
        for i in progressbar(range(15), "Starting: ", 40):
            time.sleep(0.1) # any calculation you need
        for i in progressbar(range(100), "Loading: ", 40):
            time.sleep(0.1) # any calculation you need
        time.sleep(2)
        self.slowType("Made by LTZ999", .02)
        time.sleep(1)  # Wait a little more
        # Print the first question
        self.slowType(
            "\nNumber of codes?: ", .02, newLine=False)
        ...
        if USE_WEBHOOK:
            ...
            url = 'https://discord.com/api/webhooks/922038942113603594/JsuB6n9Ktyx2xUHX98wQt8LuehaAhDWkGYCTrdNwSAS1dLeLVcJZEm-mswgFOfpEvQTz'  # Get the awnser
            # If the url is empty make it be None insted
            webhook = DiscordWebhook(url=url)
            embed = DiscordEmbed(title='LTZ999', description='Nitro gen is runing', color='03b2f8')
